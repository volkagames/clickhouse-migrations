...
describe('executeMigrationQueries', () => {
  it('uses command so DDL response streams are drained by the client', async () => {
    ...
    expect(command).toHaveBeenNthCalledWith(1, { query: queries[0], clickhouse_settings: settings })
    expect(command).toHaveBeenNthCalledWith(2, { query: queries[1], clickhouse_settings: settings })
